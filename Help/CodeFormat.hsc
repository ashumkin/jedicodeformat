...
\viewkind4\uc1\pard\cf1\b\f0\fs32 Jedi Code Format V\lang2057\f1 2.0 Beta 7\cf0\lang1031\b0\f0\fs20 
\par \cf2\{keepn\}
\par \cf0 
...
\par 
\par \cf0\f3 Last change : 1 September, 2003\f1 
\par }
12
Scribble12
Programs
...
\par e.g.\lang2057\f1  enter\lang1031\f0  \b\f2 MyUnit\lang2057\b0\f1 , \lang1031\f0 not \f2 c:\\code\\MyDelphiProject\\MyUnit.pas\f0  
\par 
...
\par \f3 TFoo = class (TBar)\f0  not
\par \f3 TFoo = class(TBar)\f0 
\par 
\par \b Spaces before colon:\b0  these options determine the number of spaces inserted\lang2057\f1  \lang1031\f0 before colons in various contexts. Borland standard style is to always put zero\lang2057\f1  \lang1031\f0 spaces before a colon, but some people like these.
\par E.g. setting \b Spaces before colon in var and const declaration\b0  to 3 would change 
\par \f3   FooStr: array[1..2] of string = ('hi', 'lo');
\par \f0 to
\par \f3   FooStr   : array[1..2] of string = ('hi', 'lo');
\par \f0 
\par 
\par \b Turn tabs to spaces:\b0  Recommended. If you change the "block indentation\lang2057\f1  \lang1031\f0 spaces", you may want to change the "spaces per tab" to match.
\par 
...
\par E.g. turning this option on and setting the number of extra spaces to \lang2057\f1 1\lang1031\f0  (block indent is still 2) produces output like:
\par 
\par \b\f3 procedure \b0 someproc;
\par \b begin\b0 
...
\par 
\par \f4 if somecond then
\par begin
\par   statemements
\par end
\par \f0 
\par But \lang2057\f1 you may also want your\lang1031\f0  cod\lang2057\f1 e lik\lang1031\f0 e \f4 if a > 3 then b := 4;\f0  \lang2057\f1 to stay as is, not\lang1031\f0  to drop \f4 b := 4\f0  onto a new line in cases where you wrote it all on one line.
\par  
...
\par \f0 
\par I used to think that this was silly\lang2057\f1 ;\lang1031\f0  too much manual labour for no benefit. Then I got the program to do it for me\lang2057\f1 ; n\lang1031\f0 ow I think it looks neat. Alignment is turned off by default as it is not part of the Borland code formatting standard.
\par 
...
FALSE
60
{\rtf1\ansi\ansicpg1252\deff0\deflang2057{\fonttbl{\f0\fnil\fcharset0 Arial;}{\f1\fnil Arial;}{\f2\fswiss Arial;}{\f3\fswiss\fcharset0 Arial;}{\f4\fnil\fcharset0 Courier New;}}
{\colortbl ;\red0\green0\blue255;\red0\green0\blue0;}
...
\par \{$ENDIF\}\f0 
\par \f1 
\par \f0 so clearly the text between the ($IFDEF ... and \{$ENDIF\} is not parsed. However the preprocessor is not usually used like this. More commonly it has uses such as platform-dependant code, e.g.
\par 
\par \f4 uses
\par \{$IFDEF MSWINDOWS\}
\par Windows,
...
\par SysConst;\f0 
\par \f1 
\par \f0 The code has well-formed structure even when the preprocessor directives are treated as regular comments. It is recommended that you write code that follows that rule. However much existing code does not do this, and does not make sense if the preprocessor directives are ignored.
\par 
\par e.g.
\par \f4 Function foo;
\par begin
...
\par \{$ENDIF\}
\par \f0  
\par Removing the directives yeilds code that will not parse:
\par 
\par \f4 Function foo;
\par begin
\par   ...
...
\par \f0  \f1 
\par 
\par \b\f0 Enable\b0 : Turn this off if you are not having any problems with the preprocessor at all. This will cause the JCF to behave as all previous versions and not notice the preprocessor directives. The formatter will attempt to parse and format all of the source code. 
\par 
\par Turn this option on if you are having parsing problems caused by preprocessor-related code. In this case, only parts of the code will be formatted, and parts that are excluded by the preprocessor will not be.  
\par 
\par You should enter the symbols defined and the options defined so that the formatter will know which preprocessor-delimited blocks to parse and which to ignore. Note that for the options, they must be entered in the form thatyou use them. E.g. if you use the preprocessor switch \f4\{$IFOPT R+\}\f0 , then the option \f4 R+\f0  must bge entered, the synonym \f4 RANGECHECK \f0 is not sufficient.
\par 
\par The \f4\{$IFDEF\f0 , \f4\{$IFNDEF\f0  and \f4\{$IFOPT\f0  directives are suported, and the new and more flexible \f4\{$IF\f0  directive is partly supported, with simple expressions working. \f4\{$DEFINE\f0  and \f4\{$UNDEF\f0  directives will be recognised and will change the working set of defined sysmbols, but only in the source file in which they are encountered.
\par 
\par 
\par }
200
